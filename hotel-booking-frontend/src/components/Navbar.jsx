  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-10 h-20 flex items-center justify-between">
                ...
                {userMenuOpen && (
                  ...
                  >
                    <div className="px-4 py-3 border-b border-gold/10">
                      <p className="font-inter text-xs text-muted">Signed in as</p>
                      <p className="font-inter text-sm text-neutral-900 font-medium truncate">{user.username}</p>
                    </div>
                    <Link
                      to="/profile"
                    ...
                    <button
                      ...
                      className="w-full flex items-center gap-2.5 text-left px-4 py-3 text-sm font-inter text-danger hover:bg-[#F6E7E2] transition-colors"
                    >
                      <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
        ...
        >
          <span className={`h-0.5 bg-neutral-800 rounded-full transition-transform duration-300 ${mobileOpen ? 'rotate-45 translate-y-2' : ''}`} />
          <span className={`h-0.5 bg-neutral-800 rounded-full transition-opacity duration-300 ${mobileOpen ? 'opacity-0' : ''}`} />
          <span className={`h-0.5 bg-neutral-800 rounded-full transition-transform duration-300 ${mobileOpen ? '-rotate-45 -translate-y-2' : ''}`} />
        </button>
      </div>
              ...
              {user ? (
                <>
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="font-inter text-sm text-neutral-900 font-medium">{user.username}</span>
                  </div>
                  <Link to="/profile" onClick={() => setMobileOpen(false)} className="font-inter text-base text-neutral-800">
                    My Bookings
                  </Link>
                  <button
                    onClick={() => {
                ...
                <>
                  <Link to="/login" onClick={() => setMobileOpen(false)} className="font-inter text-base text-neutral-800">
                    Login
                  </Link>
                  <Link
                    to="/register"
